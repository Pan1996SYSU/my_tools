...
for i in range(5):
    ...
    sht[i * const_num + 6, 5].value = '13'
    sht[i * const_num + 6, 6].value = '用气情况'
    sht[i * const_num + 6, 7].value = '日均用气量'
    sht[i * const_num + 7, 0].value = '2'
    sht[i * const_num + 7, 1].value = '工程建设资料'
    sht[i * const_num + 7, 2].value = '现场勘察图'
    sht[i * const_num + 7, 5].value = '14'
    sht[i * const_num + 7, 7].value = '用气压力'
    sht[i * const_num + 8, 0].value = '3'
    sht[i * const_num + 8, 2].value = '设计图'
    sht[i * const_num + 8, 5].value = '15'
    sht[i * const_num + 8, 7].value = '抄表情况'
    sht[i * const_num + 9, 0].value = '4'
    sht[i * const_num + 9, 2].value = '施工图'
    sht[i * const_num + 9, 5].value = '16'
    sht[i * const_num + 9, 6].value = '隐患情况'
    sht[i * const_num + 9, 7].value = '安检记录单'
    sht[i * const_num + 10, 0].value = '5'
    sht[i * const_num + 10, 2].value = '施工验收资料'
    sht[i * const_num + 10, 5].value = '17'
    sht[i * const_num + 10, 7].value = '隐患通知单'
    sht[i * const_num + 11, 0].value = '6'
    sht[i * const_num + 11, 1].value = '合同签订情况'
    sht[i * const_num + 11, 2].value = '报装合同'
    sht[i * const_num + 11, 5].value = '18'
    sht[i * const_num + 11, 7].value = '隐患整改跟进记录'
    sht[i * const_num + 12, 0].value = '7'
    sht[i * const_num + 12, 2].value = '用气协议'
    sht[i * const_num + 12, 5].value = '19'
    sht[i * const_num + 12, 7].value = '维修记录'
    sht[i * const_num + 13, 0].value = '8'
    sht[i * const_num + 13, 2].value = '相关补充协议'
    sht[i * const_num + 13, 5].value = '20'
    sht[i * const_num + 14, 0].value = '9'
    sht[i * const_num + 14, 2].value = '流量计型号'
    sht[i * const_num + 14, 5].value = '21'
    sht[i * const_num + 15, 0].value = '10'
    sht[i * const_num + 15, 2].value = '调压器型号'
    sht[i * const_num + 15, 5].value = '22'
